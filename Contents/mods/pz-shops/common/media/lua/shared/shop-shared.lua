...
function _internal.generateMoneyValue(item, value, force)
    if item ~= nil and _internal.isMoneyType(item:getFullType()) then

        if (not _internal.valuedMoney[item] or force) then
        ...
        if not isServer() and item:getModData().value then
            item:setName(_internal.numToCurrency(item:getModData().value))
        end
...
---@return string|nil
function _internal.getWalletID(playerObj)
    if not playerObj then return nil end
    if not isClient() and not isServer() then
        return "shared_wallet"
    end
    local rawSteamID = playerObj:getSteamID()
    local steamID = (rawSteamID and rawSteamID ~= 0) and string.format("%.0f", rawSteamID) or ""
    local username = tostring(playerObj:getUsername() or "")
    local hasSteam = steamID ~= ""
    local hasUser = username ~= ""
    if hasSteam and hasUser then return "mp_" .. steamID .. "_" .. username end
    if hasUser then return "user_" .. username end
    return nil
end
