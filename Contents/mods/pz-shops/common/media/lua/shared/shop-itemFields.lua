...
function itemFields.gatherFields(i, purgeHidden)
    ...
    if instanceof(item, "Food") then
        ...
        local foodSicknessChange = item:getFoodSicknessChange()
        fields.foodSicknessChange = foodSicknessChange
        if foodSicknessChange == 0 then
            hidden_fields.foodSicknessChange = true
        end
...
function itemFields.getFieldAssociatedFunctions(item)
    ...
    if food then
        ...
        fields.foodSicknessChange = "setFoodSicknessChange"
        --fields.Poison = item:isPoison()
...
---Evaluates a conditional expression string against an actual field value.
---Supported syntax (all comparisons numeric when both sides parse as numbers):
---  >N   >=N   <N   <=N       numeric / lexicographic comparison
---  N~M                        inclusive numeric range
---  !value                     not-equal
---  =~pattern                  case-insensitive substring (contains) match
---  exact                      exact equality
---@param expr string|nil       The condition expression from buyConditions
---@param actual any            The item's field value
---@return boolean
function itemFields.parseConditional(expr, actual)
    ...
    local pattern = exprStr:match("^=~(.+)$")
    if pattern then
        local hay = tostring(actual):lower()
        local needle = pattern:lower():gsub("([^%w])", "%%%1")
        return hay:find(needle) ~= nil
    end

    -- >=N
    local gev = exprStr:match("^>=(.+)$")
    if gev then
        local n, a = tonumber(gev), tonumber(actual)
        if n and a then return a >= n end
        return tostring(actual) >= gev
    end

    -- <=N
    local lev = exprStr:match("^<=(.+)$")
    if lev then
        local n, a = tonumber(lev), tonumber(actual)
        if n and a then return a <= n end
        return tostring(actual) <= lev
    end

    -- >N  (checked after >= so ">=" isn't caught here)
    local gv = exprStr:match("^>(.+)$")
    if gv then
        local n, a = tonumber(gv), tonumber(actual)
        if n and a then return a > n end
        return tostring(actual) > gv
    end

    -- <N  (checked after <= so "<=" isn't caught here)
    local lv = exprStr:match("^<(.+)$")
    if lv then
        local n, a = tonumber(lv), tonumber(actual)
        if n and a then return a < n end
        return tostring(actual) < lv
    end

    -- N~M inclusive range
    local lo, hi = exprStr:match("^(%-?[%d%.]+)~(%-?[%d%.]+)$")
    if lo and hi then
        local nLo, nHi = tonumber(lo), tonumber(hi)
        local a = tonumber(actual)
        if nLo and nHi and a then return a >= nLo and a <= nHi end
        return false
    end

    -- !value  not-equal
    local nv = exprStr:match("^!(.+)$")
    if nv then
        local nNum, aNum = tonumber(nv), tonumber(actual)
        if nNum and aNum then return aNum ~= nNum end
        if nv == "true"  then return actual ~= true  and tostring(actual):lower() ~= "true"  end
        if nv == "false" then return actual ~= false and tostring(actual):lower() ~= "false" end
        return tostring(actual):lower() ~= nv:lower()
    end

    -- exact match — numeric first, then case-insensitive string
    local exprNum, actualNum = tonumber(exprStr), tonumber(actual)
    if exprNum and actualNum then return actualNum == exprNum end

    if exprStr == "true"  then return actual == true  or tostring(actual):lower() == "true"  end
    if exprStr == "false" then return actual == false or tostring(actual):lower() == "false" end

    return tostring(actual):lower() == exprStr:lower()
end
