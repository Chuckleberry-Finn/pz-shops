...
function ISTradingUI:render()
    ...
    if SandboxVars.ShopsAndTraders.PlayerWallets then
        ...
        if offerAmount ~= self.lastSentOfferAmount then
            self.lastSentOfferAmount = offerAmount
...
function ISTradingUI:finalizeDeal()
    if SandboxVars.ShopsAndTraders.PlayerWallets then
        ...
        sendClientCommand("shop", "exchangeFunds", {otherOnlineID=self.otherPlayer:getOnlineID(), offerA=offeredAmount, offerB=otherOffer})
    end
    ISTradingUI_finalizeDeal(self)
end


local function onTradingUIUpdateState(player, state)
    if ISTradingUI.instance then
        ISTradingUI.instance.lastSentOfferAmount = nil
    end
end
Events.TradingUIUpdateState.Add(onTradingUIUpdateState)
