...
function storeWindow:onChangeStoreCash()
    ...
    if not walletID then print("WARN: onChangeStoreCash - could not derive walletID.") return end
    ...
    sendClientCommand("shop", "transferFunds", {walletID=walletID, amount=value, toStoreID=self.parent.storeObj.ID})
end
...
function storeWindow:listingIsVisible(listing)
    ...
    local showListing = itemReselling and availableItem and ifNotSellingThenBuying
    if listing.alwaysShow == true then showListing = true end
    return showListing
end
    ...
    local function addOrdered(categoryOnly)
        for _,id in ipairs(self.listingOrder) do
            ...
            if listing then
                ...
                if isCategory == categoryOnly and self:listingMatchesTab(listing) and self:listingIsVisible(listing) then
                    if not listing.id then listing.id = id end
...
function storeWindow:finalizeDeal()
        ...
        if type(_item) == "string" then
            ...
            if itemType then
                if _internal.isMoneyType(itemType) then
                    ...
                    table.insert(itemsToRemoveLocally, v.item)
                else
                    ...
                    if self.storeObj.ownerID then
                        ...
                        table.insert(itemsToSell, {itemType=itemType, fields=fields, buyCheckFields=buyCheckFields, itemID=v.item:getID()})
                        table.insert(itemsToRemoveLocally, v.item)
                    end
                end
            end
    ...
    sendClientCommand(self.player,"shop", "processOrder", { playerID=walletID, storeID=self.storeObj.ID, buying=itemToPurchase, selling=itemsToSell, money=moneyItemValueUsed, moneyItemIDs=moneyItemIDs })

    for _,item in pairs(itemsToRemoveLocally) do
        local container = item:getContainer()
        if container then container:DoRemoveItem(item) end
    end
end
