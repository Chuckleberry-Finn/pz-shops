...
function getWallet(player)
    if player then
        local walletID = _internal.getWalletID(player)
        if walletID and CLIENT_WALLETS[walletID] then
            return CLIENT_WALLETS[walletID]
        end
    end
end
...
function getOrSetWalletID(playerObj)
    ...
    if not playerModData then print("WARN: Player created without modData - can't assign walletID.") return end

    local walletID = _internal.getWalletID(playerObj)
    if not walletID then print("WARN: Could not derive walletID for player.") return end

    playerModData.walletID = walletID

    local playerWallet = CLIENT_WALLETS[walletID]
    ...
    if not playerWallet or (playerWallet and (not playerWallet.playerUsername or forceMoneyOut==true)) then
        sendClientCommand(playerObj, "shop", "getOrSetWallet", {walletID=walletID, steamID=playerObj:getSteamID(), playerUsername=playerObj:getUsername()})
    end
...
function safelyRemoveMoney(moneyItem, player)
    ...
    if container then
        local outermostParent = moneyItem:getOutermostContainer() and moneyItem:getOutermostContainer():getParent()
        local isPlayerInventory = instanceof(outermostParent, "IsoPlayer")
        if isClient() and container:getType()~="floor" then
            if isPlayerInventory then
                sendClientCommand("shop", "removeMoneyItem", {itemID=moneyItem:getID()})
            else
...
local function onPlayerDeath(playerObj)
    ...
    local walletID = _internal.getWalletID(playerObj)
    if not walletID then print("WARN: onPlayerDeath - could not derive walletID.") return end
    ...
    if transferAmount > 0 then
        sendClientCommand("shop", "transferFunds", {walletID=walletID, amount=(0-transferAmount)})
        sendClientCommand("shop", "createMoneyOnDeath", {walletID=walletID, amount=transferAmount})
    end

    if SandboxVars.ShopsAndTraders.PlayerWalletsLostOnDeath == true then
        CLIENT_WALLETS[walletID] = nil
        sendClientCommand("shop", "scrubWallet", {walletID=walletID})
    end
end
    ...
    if button.internal == "OK" then
        if canManipulateMoney(item, playerObj) then
            ...
            if not item then
                local walletID = _internal.getWalletID(playerObj)
                if walletID then
                    sendClientCommand("shop", "transferFunds", {walletID=walletID, amount=(0-transferValue), forceCash=true})
                end
            end
    ...
    if (draggingOld ~= nil) and (draggingFocusOld == self) and (draggingFocusOld ~= nil) then
        ...
        if pushToActual and _internal.isMoneyType(pushToActual:getFullType()) then
            ...
            for _,money in pairs(moneyFound) do
                ...
                table.insert(removedIDs, money:getID())
            ...
            sendClientCommand("shop", "stackMoney", {
                targetItemID = pushToActual:getID(),
                newValue = ptValue+consolidatedValue,
